...
export class ProfileFormPage implements OnInit, OnChanges {
  ...
  async ngOnInit() {
    if (
      ...
      this.partyContactProfile =
        this.navigation.extras.state["partyContactProfile"];
    } else {
      this.partyContactProfile =
        await this.psprof.getPartyContactProfileAsync();
    }
    console.log(
      "ProfileFormPage::ngOnInit() this.partyContactProfile gtest",
  ...
  ngOnChanges(changes: { [propName: string]: SimpleChange }) {
    if (
      (changes["toggled"].previousValue == false ||
        changes["toggled"].previousValue == true) &&
      changes["toggled"].previousValue != changes["toggled"].currentValue
    ) {
      if (this.rForm.status == "VALID") {
        this.saveForm(this.rForm.value);
        this.emitEvent(true);
      } else {
        this.presentAlert();
      }
  ...
  buildForm() {
    this.rForm = this.formBuilder.group({
      ...
      addrLine1: [null, Validators.required],
      addrLine2: "",
      unitNumber: "",
      pobox: "",
      cityLine: "",
  ...
  saveForm(post) {
    ...
    this.partyContactProfile.postalAddress.address1 = post.addrLine1;
    this.partyContactProfile.postalAddress.address2 = post.addrLine2;
    this.partyContactProfile.postalAddress.unitNumber = post.unitNumber;
    this.partyContactProfile.postalAddress.pobox = post.pobox;
    ...
    this.partyContactProfile.postalAddress.postalCode = post.postalCode;
    this.partyContactProfile.postalAddress.latitude = post.latitude;
    this.partyContactProfile.postalAddress.longitude = post.longitude;
